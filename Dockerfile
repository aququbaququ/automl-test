...
RUN (apt-get update && \
apt install fish magic-wormhole jq && \
mkdir -p ~/.config/fish && \
mkdir -p ~/.local/bin && \
...
wget https://github.com/cli/cli/releases/download/v2.52.0/gh_2.52.0_linux_amd64.deb && apt install ./gh_2.52.0_linux_amd64.deb && rm ./gh_2.52.0_linux_amd64.deb && \
gh auth setup-git && \
